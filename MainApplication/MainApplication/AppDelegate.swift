...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func setupTabbar() {
        ...
        controllerArray = [self.discoverNavi] as? [UIViewController] ?? []
...
//        let insets = UIEdgeInsets(top: inset, left: 0, bottom: -inset, right: 0)
        ...
        let iconHome = UITabBarItem(title: nil, image: UIImage.init(icon: .regular(.compass), size: iconSize), selectedImage: UIImage.init(icon: .regular(.compass), size: iconSize))
        self.discoverNavi?.tabBarItem = iconHome
